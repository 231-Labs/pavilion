// src/components/KioskClientProvider.tsx
...
export function KioskClientProvider({ children, networkName = 'testnet' }: { children: React.ReactNode; networkName?: 'testnet' | 'mainnet' }) {
  const suiClient = useSuiClient();
  const kioskNetwork = networkName === 'mainnet' ? Network.MAINNET : Network.TESTNET;

  const kioskClient = useMemo(() => {
    return new KioskClient({ client: suiClient as any, network: kioskNetwork });
