...
import { fetchModels, ModelFile } from '../../lib/services/walrus-client';
...
export const GLBLoader: React.FC<GLBLoaderProps> = ({ sceneManager }) => {
  ...
  const loadAllModels = async () => {
    ...
    try {
      ...
      const modelsToLoad = files.map((file: ModelFile, index: number) => ({
        url: file.url,
        options: {
      ...
      const modelNames = modelsToLoad.map((m: { url: string; options: { name: string; position: { x: number; y: number; z: number } } }) => m.options.name);
